...
function findFloorAndCeil1(node, target) {

  function traverse(node, floor = null, ceil = null) {
    if (!node) return [floor, ceil];
    ...
    if (target < node.value) {
      ceil = node.value;
      return traverse(node.left, floor, ceil);
    }

    if (target > node.value) {
      floor = node.value;
      return traverse(node.right, floor, ceil)
    }
  }
